...
图表契约已移出本技能。`chart_spec` 的类型枚举、字段要求、图表规则和前端渲染边界，
见 `chart-visualization` 技能的 `reference/10-chart-spec-contract.md`。

本技能只负责把查询结果表达为 `sql_execution`；是否成图、如何成图由图表技能决定。
