...
**Date:** 2026-03-26  
**Goal:** 将仓库内置 `dataagent-nl2sql` 收敛为 OpenDataWorks 平台通用问数 skill，把租户私有知识从内置 skill 中拆出，并为后续多 skill 扩展保留清晰边界。
...
- 覆盖回归测试、设计文档和实施计划
- 在仓库根目录创建一个本地未提交的 `business-domain-assistant` skill，用来承接拆出的业务知识

不在范围内：

- 不实现扩展 skill 上传压缩包
- 不实现扩展 skill 启用/停用管理
- 不让根目录 `.claude/skills/business-domain-assistant` 进入当前 DataAgent 运行时发现链路
- 不新增 registry 表、上传入口或 settings schema
...
- 当前内置 `dataagent-nl2sql` 同时承载：
  ...
  - 数据中台通用问数规则
  - 租户私有的环境、对象、示例和默认口径
- 这些私有知识曾混入 `assets/*`、`reference/*` 和 `SKILL.md`
...
- 被移除的业务知识转移到仓库根目录本地 skill：
  - `.claude/skills/business-domain-assistant`
  - 该 skill 不提交到 Git
  - 当前运行时不自动发现它
- 后端内部明确区分：
  - 内置 skill 根目录
...
- 汇总脚本本身改成“平台术语 / 通用规则”口径，避免重新引入“租户私有术语”语义
...
### 5. Local business-domain skill

- 通过 `skill-creator` 在仓库根目录初始化：
  - 目录名：`business-domain-assistant`
  - 展示名：`业务知识 Skill`
- 仅保留知识 overlay：
  - 租户私有术语资料
  - 租户私有命名资料
...
- DataAgent 运行时：
  ...
  - 不发现根目录 `.claude/skills/business-domain-assistant`

## Risks

- 移除租户私有 few-shot 后，某些历史问题会从“自动代入租户私有默认值”变成“基于 metadata 的弱回答或追问”
...
- 本地 smoke 环境可用时：
  - 平台通用题可跑通
  - `df/di` 规则仍生效
  - 租户私有知识模糊题不再使用内置租户私有默认值
- 本地 `business-domain-assistant` skill：
