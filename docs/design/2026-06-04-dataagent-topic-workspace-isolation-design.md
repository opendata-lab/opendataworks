...
Enabled skills are exposed through `.claude/skills`. The workspace helper copies each enabled skill folder from the discovery root into `<workspace>/.claude/skills/<folder>` as a real directory. Copies are used instead of symlinks because some Claude Code / Agent SDK skill discovery does not follow symlinked skill directories. The copy is materialized once per topic, not on every message: a subsequent `prepare` re-copies only when the source skill is newer (newest file mtime), so a same-name re-import or edit self-heals while unchanged sources are left untouched. When the discovery root and the workspace skills directory are the same path (e.g. the sandbox child where the skill is bind-mounted at `/app/.claude/skills/<folder>`), the copy is skipped because the skill is already in place.
