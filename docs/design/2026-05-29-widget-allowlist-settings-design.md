...
- `skill_admin_service`：
  ...
  - `_runtime_settings_payload()` 以 `WIDGET_ALLOWED_SITES_JSON` 作为默认值（env 作为兜底）。
...
`_allowed_widget_sites()` 改为读取 `current_settings_payload()` 的 `widget_allowed_sites`。该函数本身已经把 env 默认值与持久化记录合并，因此结果即“生效白名单”：env 为默认，DB 覆盖（含清空）。读取失败时返回空列表（请求被拒，安全侧默认关闭）。
...
- 向后兼容：未保存过白名单的环境继续使用 `WIDGET_ALLOWED_SITES_JSON`；保存后以 DB 为准。
