...
3. **Start Services**:
   ...
   - `dataagent-home-init` 是一次性 init 服务（以 root 运行），在 backend/runner 启动前预创建 `DATAAGENT_HOME_HOST_DIR` 下的 `.dataagent/runtime/{topics,enabled-skills}` 目录，并把属主改成 `DATAAGENT_RUNTIME_UID/GID`。这避免了 Docker 把缺失的 bind mount 宿主目录默认建成 `root:root 755`、导致非 root 的 `dataagent-backend` 写 `/tmp/dataagent-home/.dataagent` 时报 `permission denied` 的问题；若改了 `DATAAGENT_RUNTIME_UID/GID`，该 init 服务会自动按新值修正属主
