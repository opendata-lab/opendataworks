...
3. **Start Services**:
   ...
   - `dataagent-home-init` 是一次性 init 服务（以 root 运行），在 backend/runner 启动前确保 `DATAAGENT_HOME_HOST_DIR`（含 `.dataagent/runtime/topics` 这一 topic 级工作区根目录）存在，并把属主改成 `DATAAGENT_RUNTIME_UID/GID`。这避免了 Docker 把缺失的 bind mount 宿主目录默认建成 `root:root 755`、导致非 root 的 `dataagent-backend` 写 `/tmp/dataagent-home/.dataagent` 时报 `permission denied` 的问题；属主修好后，每个 topic/agent 的具体 skill 目录仍由运行时按需在各自 topic 工作区下创建，init 不预设任何按 skill 划分的目录；若改了 `DATAAGENT_RUNTIME_UID/GID`，该 init 服务会自动按新值修正属主
