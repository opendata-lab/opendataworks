...
3. **Start Services**:
   ...
   - DataAgent backend/runner 额外持久化宿主目录 `DATAAGENT_HOME_HOST_DIR`（默认 `/workspaces`），同时作为服务容器内 `HOME` 与 bind mount 挂载点。topic 级工作区默认位于 `/workspaces/<topic_id>/`，Claude SDK session 文件落在该 topic 根目录的 `.claude/projects/<sanitized-cwd>/` 下；同一 topic 多轮复用该目录，不同 topic 不共享 `.claude` 状态
   - `dataagent-home-init` 是一次性 init 服务（以 root 运行），在 backend/runner 启动前确保 `DATAAGENT_HOME_HOST_DIR`（即 `/workspaces`，topic 工作区根目录）存在，并把属主改成 `DATAAGENT_RUNTIME_UID/GID`。这避免了 Docker 把缺失的 bind mount 宿主目录默认建成 `root:root 755`、导致非 root 的 `dataagent-backend` 写 `/workspaces` 时报 `permission denied` 的问题；属主修好后，每个 topic/agent 的具体 skill 目录仍由运行时按需在各自 topic 工作区下创建，init 不预设任何按 skill 划分的目录；若改了 `DATAAGENT_RUNTIME_UID/GID`，该 init 服务会自动按新值修正属主
   ...
   - task child 内部路径与 backend/runner 服务路径分离：`/mnt/workspace` 是当前 task workspace 和 project skills 目录，`/mnt/home` 是 child-local tmpfs Claude HOME，`/tmp` 只用于临时 scratch；child 不注入 `DATAAGENT_WORKSPACE_DIR`、`DATAAGENT_WORKSPACE_PREPARED`、`DATAAGENT_SANDBOX_ROOT`
