...
3. **Start Services**:
   ...
   - DataAgent 额外持久化宿主目录 `DATAAGENT_HOME_HOST_DIR`（默认 `/workspaces`），同时作为容器内 `HOME` 与 bind mount 挂载点。topic 级工作区默认位于 `/workspaces/<topic_id>/`，Claude SDK session 文件落在该 topic 根目录的 `.claude/projects/<sanitized-cwd>/` 下；同一 topic 多轮复用该目录，不同 topic 不共享 `.claude` 状态
